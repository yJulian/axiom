...
Axi4MasterEngine::driveR()
{
    if (rInFlightSeq_ != UINT64_MAX) {
        ...
        if (it != reads_.end() && pins_.axiMasterGetRReady()) {
            // Single-beat response per transaction in this scaffold
            // (matches the fixed 8-byte-per-beat convention used
            // elsewhere); a burst-of-many-beats read completion would
            // stream `it->second.data` here beat by beat instead.
            readOrder_[it->second.id].pop_front();
            reads_.erase(it);
    ...
    auto &x = reads_.at(seq);
    uint64_t data = 0;
    std::memcpy(&data, x.data.data(),
                std::min<size_t>(sizeof(data), x.data.size()));
    pins_.axiMasterSetRId(x.id);
    pins_.axiMasterSetRData(data);
    pins_.axiMasterSetRResp(static_cast<uint8_t>(AxiResp::Okay));
    pins_.axiMasterSetRLast(1);
